...
struct grid {
        ...
        array<array<int, 16>, 16> reverse_manhattan_distances = {}; // for backtracking
...
public:
    ...
    void floodfill();
    void floodfillToStart();           // reverse BFS to start (0,0)
    int reverse_getNextMove(int row, int col);
    void final_floodfill();            // BFS constrained to visited cells only
